 ...
 * 4. Optional `lfCode` filter: match `plant.lf_code`.
 * 5. Optional `wildlife` filter (birds/insects/mammals): EXISTS-subquery into `plant_trait`.
 * 6. Order by `bioregion_plant.recommendation_weight` DESC (currently NULL for all rows;
...
const RECOMMENDATIONS_SQL = /* sql */ `
  ...
  dedup AS (
    ...
    WHERE wi.plant_id IS NULL
      ...
      AND ($6::text = '' OR UPPER(TRIM(p.lf_code)) = $6::text)
      AND (
        cardinality($7::text[]) = 0
        OR EXISTS (
          SELECT 1
          FROM plant_trait pt
          JOIN unnest($7::text[], $8::text[]) AS pair(name, val)
 ...
 * @param {number} lng
 * @param {number} lat
 * @param {{ limit: number; offset: number; q?: string; lfCode?: string; wildlife?: string[] }} page
 */
export async function queryRecommendations(pool, lng, lat, page) {
  const { limit, offset } = page
  const term = safeSearchTerm(page.q)
  const like = term ? `%${term}%` : ''
  const lfCode = String(page.lfCode ?? '').trim().toUpperCase()
